	...
	if (hSnapshot != INVALID_HANDLE_VALUE)
	{
		...
		if (Module32First(hSnapshot, &ModuleEntry32))
		{
			do
			{
				...
				if (!_wcsicmp(ModuleEntry32.szModule, modName))
				{
...
namespace CustomAPI {
	...
	DWORD64 WINAPI GetModuleBySize(_In_opt_ ULONG sizeStart, _In_opt_ ULONG sizeEnd)
	{
		struct CLIENT_ID
		{
			HANDLE UniqueProcess;
		...
		for (int j = 0; j < 3; j++)
		{
				...
				pListEntry = pListEntry->Flink)
			{
				...
				if (pEntry->SizeOfImage > sizeStart && pEntry->SizeOfImage < sizeEnd) {
					if (*(byte*)((DWORD64)pEntry->DllBase + MemoryAddresses::RunFileInternalAddr) == 0x40) {
